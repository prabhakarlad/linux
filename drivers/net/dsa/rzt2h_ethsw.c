				      ...
				      bool rx_pause)
{
	...
	if (dp->index != ETHSW_CPU_PORT)
		miic_switchcore_adjust(ethsw->pcs[dp->index], duplex, speed);
...
static int ethsw_pcs_get(struct ethsw *ethsw)
{
	...
	for_each_available_child_of_node(ports, port) {
		...
		pcs = miic_create(ethsw->dev, pcs_node);
		if (IS_ERR(pcs)) {
...
static int ethsw_probe(struct platform_device *pdev)
{
	...
	ethsw->reset = devm_gpiod_get_optional(&pdev->dev, "phy-reset", GPIOD_OUT_HIGH);
	...
	ethsw->clk = devm_clk_get(dev, "clk");
	if (IS_ERR(ethsw->clk)) {
